...
export default function SelectPage() {
    ...
    const handleStartPractice = () => {
        if (!selected.caseId || !selected.caseName || !selected.checklistId) return;

        startTransition(() => {
            router.push(
                `/record-select/cpx?category=${encodeURIComponent(
                    selected.chiefComplaint
                )}&case=${encodeURIComponent(selected.caseName!)}&checklistId=${encodeURIComponent(selected.checklistId!)}`
            );
        });
    };
